...
def main():
    ...
    logging.info("Serverul a pornit pe adresa {} si portul {}".format(adresa, port))
    ...
    while True:
        ...
        try:
            ...
            if 'SYN' in flags:
                ...
                if 'FIN' in flags:
                    logging.info("send fin ack")
                    msg_fara_check = create_header(seq_nr, seq_nr_client, 0, ['FIN', 'ACK'])
                    check = calculeaza_checksum(msg_fara_check)
                    msg = create_header(seq_nr, seq_nr_client, check, ['FIN', 'ACK'])

                    sock.sendto(msg,address)

                    data, address = sock.recvfrom(MAX_SEGMENT)
                    logging.info('received ack for fin from client')
                    if not verifica_checksum(data):
                        continue
                    _, ack_nr, _, flags, _ = parse_header(data)
                    if 'ACK' in flags and ack_nr == seq_nr:
                        break
                else:
