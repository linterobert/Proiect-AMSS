...
def connect(sock, adresa_receptor):
    ...
    return ack_nr == seq_nr  # return true only when 3way handshake is success
...
def finalize(sock, adresa_receptor):
    ...
    while True:
        if send(sock, adresa_receptor, octeti_payload=b"", flags=["FIN"]):
            break
...
def send(sock, adresa_receptor, octeti_payload, flags=['PSH']):
    ...
    '''
    logging.info("payload: " + str(octeti_payload))
    global seq_nr
    ...
    while True:
        try:
            ...
            if 'FIN' in flags:
                ...
                if not verifica_checksum(data):
                    return False
                seq_nr_server, ack_nr, _, flags, _ = parse_header(data)
                if 'ACK' not in flags or 'FIN' not in flags or ack_nr != seq_nr:
                    return False
                hdr = create_header(seq_nr, seq_nr_server, 0, ['ACK'])
                checksum = calculeaza_checksum(hdr)
                hdr = create_header(seq_nr, seq_nr_server, checksum, ['ACK'])
                sock.sendto(hdr, adresa_receptor)
                return True
...
def main():
    ...
    try:
        ...
        while True:
            if connect(sock, adresa_receptor):
                break

        file_descriptor = open(fisier, 'rb')
        for segment in citeste_segment(file_descriptor):
            logging.info("segment citit: " + str(segment))
            send(sock, adresa_receptor, segment)
